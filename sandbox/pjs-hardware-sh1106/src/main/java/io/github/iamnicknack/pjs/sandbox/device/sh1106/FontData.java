 ...
 * <a href="https://github.com/adafruit/Adafruit-GFX-Library/blob/master/glcdfont.c">Adafruit-GFX-Library</a>
 */
public class FontData {
    ...
    private static final int[] INT_DATA = {
            ...
            0x20, 0x7A, 0x38, 0x54, 0x54, 0x55, 0x59, 0x21, 0x55, 0x55, 0x79, 0x41, 0x22, 0x54, 0x54, 0x78,
            0x42, // a-umlaut
    ...
    public static final byte[] INVERSE;

    static {
        var bytes = new byte[INT_DATA.length];
        var bytesInverse = new byte[INT_DATA.length];

        for (int i = 0; i < INT_DATA.length; i++) {
            bytes[i] = (byte) INT_DATA[i];
            bytesInverse[i] = (byte) ~INT_DATA[i];
        }

        DATA = bytes;
        INVERSE = bytesInverse;
    }

    public static byte[] getCharacterData(int c) {
        var index = ((c < 250) ? c : 0) * 5;
        var bytes = new byte[6];
        System.arraycopy(DATA, index, bytes, 0, 5);
        return bytes;
    }

    public static byte[] getInverseCharacterData(int c) {
        var index = ((c < 250) ? c : 0) * 5;
        var bytes = new byte[6];
        System.arraycopy(INVERSE, index, bytes, 0, 5);
        return bytes;
    }
}
