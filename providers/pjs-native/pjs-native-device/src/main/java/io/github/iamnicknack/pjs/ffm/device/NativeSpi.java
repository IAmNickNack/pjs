...
public class NativeSpi implements Spi, AutoCloseable {
    ...
    public int transfer(byte[] write, int writeOffset, byte[] read, int readOffset, int length) {
        ...
        var segment = nativeContext.segment(transfer, SpiTransfer.class);
        ioctlOperations.ioctl(fileDescriptor.fd(), SpiConstants.SPI_IOC_MESSAGE(1), segment, MemorySegment.class);
