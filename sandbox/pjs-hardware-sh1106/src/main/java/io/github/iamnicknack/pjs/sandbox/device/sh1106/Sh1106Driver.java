...
public interface Sh1106Driver {
    /**
     * Known-acceptable command sequence for initialisation.
     */
    CommandSequence DEFAULT_STARTUP_SEQUENCE = new CommandSequence()
     ...
     */
    void command(CommandSequence commandSequence);
     ...
     */
    default void command(Command command, int operand) {
        command(new CommandSequence().append(command, operand));
    }
     ...
     */
    void display(byte[] data, int offset, int length);

    /**
     * Send display data to the device. Data is written to the specified page and column.
     ...
     * @param length the number of bytes to write
     */
    void display(int page, int column, byte[] data, int offset, int length);
